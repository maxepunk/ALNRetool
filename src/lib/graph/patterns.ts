...
export function testParser(): void {
  const testCases = [
    {
      ...
      text: 'This is a memory token. SF_RFID: [MEM-001] SF_ValueRating: [3] SF_MemoryType: [Business] SF_Group: [CEO Memories (x2)]',
    },
    {
      name: 'Partial pattern',
      text: 'SF_RFID: [TEST-002] SF_ValueRating: [5]',
    },
    {
      name: 'No patterns',
    ...
    {
      name: 'Invalid values',
      text: 'SF_ValueRating: [10] SF_MemoryType: [Unknown]',
    },
  ];
  
  console.group('SF_ Pattern Parser Test');
  ...
  testCases.forEach(({ name, text }) => {
    ...
    console.log('Formatted:', formatSFMetadata(result.metadata));
    console.groupEnd();
  });
